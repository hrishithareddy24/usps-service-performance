...
# ============================================
# DATA LOADING — all from GCP, fully dynamic
# ============================================
@st.cache_data(ttl=3600)  # refresh every hour automatically
def load_data():
    client = storage.Client(project='project-4be3a115-f3f9-404e-8ba')
    ...
    rural_urban['rural_urban'] = rural_urban['rural_urban'].map(
        {'Yes': 'Rural', 'No': 'Urban'}
    )
    ...
    district['avg_days']      = district['avg_days'].astype(float)

    # Clean mailtype
    mailtype = mailtype.dropna(subset=['rural_urban']).copy()
    mailtype['avg_score_pct']      = mailtype['avg_score'] * 100
    mailtype['rural_urban_label']  = mailtype['rural_urban'].map(
        {'Yes': 'Rural', 'No': 'Urban'}
    )
...
col1.metric(
    "Rural On-Time Rate",
    f"{rural['avg_score_pct']:.1f}%",
    f"{rural['avg_score_pct'] - fy26_target:.1f}% vs FY26 Target",
    delta_color="inverse"
)
col2.metric(
    "Urban On-Time Rate",
    f"{urban['avg_score_pct']:.1f}%",
    f"{urban['avg_score_pct'] - fy26_target:.1f}% vs FY26 Target",
    delta_color="inverse"
)
col3.metric(
    "Rural vs Urban Gap",
...
col5.metric(
    "Total Records",
    f"{rural_urban['total_records'].sum()/1e6:.0f}M",
    "rows analyzed"
)
...
# ============================================
# ROW 2 — Rural vs Urban Comparison
# ============================================
...
with col1:
    ...
    fig1.update_layout(showlegend=False,
                       yaxis_range=[
                           rural_urban['avg_score_pct'].min() - 5,
...
# ============================================
# ROW 3 — Worst Districts (fully dynamic)
# ============================================
...
fig3 = px.bar(
    district_filtered,
    x='avg_score_pct', y='orgn_dist_name',
    orientation='h',
    color='avg_score_pct',
...
fig3.update_layout(coloraxis_showscale=False,
                   xaxis_range=[
                       district_filtered['avg_score_pct'].min() - 5, 95
                   ])
...
# ============================================
# ROW 4 — Mail Type (fully dynamic)
# ============================================
st.subheader("Performance by Mail Type")

mail_filter = st.selectbox(
    "Filter by area type",
    ['Both', 'Rural Only', 'Urban Only']
)
...
top_types = mailtype_filtered.groupby('mail_simple')['records'] \
    .sum().nlargest(10).index
mailtype_filtered = mailtype_filtered[
    mailtype_filtered['mail_simple'].isin(top_types)
]

fig4 = px.bar(
    mailtype_filtered,
    x='mail_simple', y='avg_score_pct',
    color='rural_urban_label',
    barmode='group',
    color_discrete_map={'Rural': '#E8593C', 'Urban': '#1D9E75'},
    labels={
        'avg_score_pct': 'On-Time Rate (%)',
        'mail_simple': 'Mail Type',
        'rural_urban_label': ''
    }
)
...
st.markdown("---")
st.caption(
    "Data source: USPS Service Performance Dashboard (spm.usps.com) | "
    "Rural classification: HRSA FORHP | "
    "Pipeline: Apache Spark on GCP Dataproc | "
    "Team Nexus Ninjas | Challenge X | GMU"
)
